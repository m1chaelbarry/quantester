"""Pure technical indicators shared by strategies and charts.
...
These helpers are pure functions on price series. Live strategies that call
them must still only see bars through the DataHandler temporal firewall
(compute on ``get_latest_bars`` windows, never on a full future-aware frame).
``quantester.visualization.indicators`` re-exports this package for charts.
